...
import re, copy
from logic import Variable, Expression, SubstituteBindingsI
...
class FSHier(dict):
    ...
    def __init__(self):
        dict.__init__(self)
#        dict.__init__(self, [(label, FeatStruct(tp, label=label, fsh=self)) for label, tp in labels_types])
...
# FSH = FSHier()
# """A dictionary representing a hierarchy of feature structure types."""

def is_anc(anc, child):
    """Is anc an ancestor in the FS hierarchy of child?"""
#    print 'Anc', anc, 'child', child
    if anc in child._types:
#      utils.some(lambda t: anc is t, child._types):
        return True
    else:
...
#     forward[id(anc)] = child
    ...
    for fname, fval2 in sorted(anc.items()):
        if fname in child:
#            print 'Fname in child'
            child[fname] = _inherit_feature_values(fname, child[fname], fval2, bindings,
                                                   forward, trace, path+(fname,), indent=indent)
        else:
#            print 'Fname not in child, fname:', fname, 'value:', fval2
#            print 'Child', child, 'frozen?', child._frozen
...
def _inherit_feature_values(fname, fval1, fval2, bindings, forward, trace, fpath, indent=0):
    ...
    """
#    print 'CALLING _inherit_fv with fname', fname, 'fval1', fval1, 'fval2', fval2
...
#    if trace and isinstance(result, FeatStruct):
...
##    @staticmethod
##    def get_from_hier(label):
##        if label not in FSH:
##            raise KeyError(label)
##        return FSH[label]
##
##    @staticmethod
##    def parse_types(*labels_types):
##        """Parse each type FS and add it to the hierarchy FSH with label."""
##        parser = FeatStructParser()
##        for label, tp in labels_types:
##            fs = FeatStruct(tp, label=label)
##            fs.add_to_hier(label)
    ...
    def _equal(self, other, check_reentrance, visited_self,
        ...
        # containing self.  Similarly for other.
        if check_reentrance:
        ...
        # then return false.
        for (fname, self_fval) in list(self.items()):
    ...
    def _remove_variables(self, visited):
        if id(self) in visited: return
        visited.add(id(self))
        for (fname, fval) in list(self.items()):
    ...
    def __repr__(self, short=False):
        ...
        """
        return self._repr(self._find_reentrances({}), {}, short=short)
    ...
    def string_list(self, long=True):
        ...
        strings = []
        if long:
            for feat, value in list(self.items()):
                s = ''
    ...
    def _repr(self, reentrances, reentrance_ids, short=False):
        """
        ...
        for (fname, fval) in sorted(items):
            ...
            if reentrance_ids.has_key(id(fval)):
                ...
                fval_repr = fval._repr(reentrances, reentrance_ids, short=short)
                if fval_repr:
    ...
    def _str(self, reentrances, reentrance_ids):
        ...
        if len(self) == 0:
            ...
            if types:
                brack = '[' + type_string + ']'
            if reentrances[id(self)]:
                return ['(%s) %s' % (reentrance_ids[id(self)], brack)]
            else:
                return [brack]
...
class _UnificationFailure(object):
    def __repr__(self): return 'featstruct.UnificationFailure'
...
def unify(fstruct1, fstruct2, bindings=None, trace=False,
    ...
    @param rename_vars: If true, then rename any variables in
        ...
        If you intend for variables in C{fstruct1} and C{fstruct2} with
...
def _destructively_unify(fstruct1, fstruct2, bindings, forward,
    ...
    if _is_mapping(fstruct1) and _is_mapping(fstruct2):
        ...
        # Unify the type sets
        if isinstance(fstruct1, FeatStruct) and isinstance(fstruct2, FeatStruct):
            fstruct1._types = merge_types(fstruct1._types, fstruct2._types)
...
def _apply_forwards(fstruct, forward, fs_class, visited):
    ...
    the target of its forward pointer (to preserve reentrancy).
    """
    # Follow our own forward pointers (if any)
...
def unify_dicts(x, y):
    ...
    for k in set(x.keys()) | set(y.keys()):
        ...
        x_val, y_val = x.get(k, None), y.get(k, None)
        if x_val != None:
            if y_val != None:
                # If x and y both have a value for k, try to unify the values
                # Treat sequence and position constraints specially
