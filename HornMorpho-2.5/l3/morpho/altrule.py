...
#                print('Making states for context copy', lc_copy, 'starting at', init_copy)
            ...
            current, change_fail, index = self.change.make_states(pre_change, not self.rc.empty,
                                                                  mult=change_mult,
...
#                print('Making copy of change state')
                ...
                change_copy_end, ignore, change_copy_index = change_copy.make_states(lc_copy_end, not self.rc.empty,
                                                                                     mult=change_mult,
                                                                                     lc_copy_init=init_copy)
...
#        print('MAKING CHANGE states{}, starting from {}'.format(' (copy)' if self.copy else '',
#                                                                 pre_change))
...
#            print('Do change multiple times, including LC; loop from', current, 'to', lc_copy_init)
        ...
        return current, change_fail, index
...
#                print('Excluding', change_chars, 'from', self)
    ...
    def make_fail_states(self, change_fail, init_state, last_change=None):
        index = 0
        current = change_fail
        pending_arc = None
        fail_state = None
...
#                        print('Current current fail state', current, 'to RC state', new_state, 'with', succ_string)
...
#                        print(' Left bounded so make fail state', fail_state, 'with', fail_chars)
                        fail_arc = fail_chars
...
#                        print('Make an arc back to first state with fail chars', fail_chars)
                ...
                if not next_element:
#                    print('Element', element, 'should be final so making non-final state for it')
                    new_state, index = self.make_state(index, 'f')
                    self.fst.add_arc(current, new_state, succ_string, succ_string)
                    # new_state must be followed by something
#                    print(' and making final state after that')
...
#        print(' Combining fail chars', fail_chars, 'with last change chars', last_change_string)
    ...
    def make_states(self, current, change_fail=None, sep_start=False, nochange='',
                    last_change=None, init_state=None):
        '''change_fail is a state for failures of the change; only relevant for the
        right context.'''
        imm_seq = []
        pending_arc = None
        index = 0
...
#        print('MAKING CONTEXT states {}{}, starting from {}'.format('(left)' if self.left else '(right)',
...
#                    print('Creating pending arc from', pending_arc[0], 'to', new_state)
                ...
                current = new_state
        if change_fail:
            self.make_fail_states(change_fail, init_state, last_change)
        return current, index
