...
import re, os, copy, time, functools
...
EP_FILTER_DIR = os.path.join(os.path.dirname(__file__))
...
class FSTCascade(list):
    ...
    def load(filename, seg_units=[], create_networks=True, subcasc=None, language=None,
        ...
        """
        if verbose:
            print('Loading FST cascade from', filename, 'for', language)
...
#        print('Weight to parse', weight)
    ...
    def show_arcs(self):
        ...
        for a in self.arcs():
            print(a, self._src[a], self._dst[a], self._in_string[a], self._weight[a])
    ...
    def del_state(self, label):
        ...
        for arc in self._incoming[label]:
            if arc in self.arcs():   # It may have been deleted already (MG)
    ...
    def inverted(self):
        ...
        fst = self.copy(FST.del_suffix(self.label, '.') + '_inv')
    ...
    def restore(fst_name, cas_directory='', fst_directory='',
                cascade=None, weighting=UNIFICATION_SR, seg_units=[], empty=True,
                phon=False, segment=False,
                generate=False, simplified=False, create_weights=True,
                verbose=False):
        '''Restore an FST from a file, in some cases creating the cascade first (MG).
        ...
        if generate:
            name += 'G'
#        if verbose:
#            print('Attempting to restore', name)
        # In other cases we need the cascade
#        if not cascade:
#            path = os.path.join(cas_directory, name + '.cas')
#            if os.path.exists(path):
#                cascade = FSTCascade.load(path,
#                                          seg_units=seg_units, create_networks=False,
#                                          verbose=verbose)
#        if cascade != None:
        ...
        if os.path.exists(os.path.join(fst_directory, filename)):
            ...
            return FST.restore_parse(fst_directory, filename, cascade=cascade, weighting=weighting,
                                     seg_units=seg_units,
    ...
    @staticmethod
    def restore_parse(directory, fst_file, weighting=None, cascade=None,
                      seg_units=[], create_weights=True, verbose=False):
        """Restore an FST from a .fst file (MG)."""
        label, suffix = fst_file.split('.')

        s = open(os.path.join(directory, fst_file), encoding='utf-8').read()
        ...
        while lines:
            ...
            if line[0] == '{' and line[-1] == '}':
                # line is a dictionary, which must the stringsets for the FST
...
#                print('Read in stringsets for', fst)
    ...
    @staticmethod
    def load(filename, cascade=None, weighting=None, seg_units=[], verbose=False, lex_features=False,
             dest_lex=False, weight_constraint=None):
        """
        Load an FST from a file (modified significantly by MG).
        ...
        if suffix == 'fst':
            if verbose:
                print('Loading FST from', filename)
            ...
            if verbose:
                print('Loading morphotactics from', filename)
            ...
            if verbose:
                print('Loading sublexicon from', filename)
            return FST.tree_to_fst(treeify_file(filename, seg_units=seg_units,
                                                features=lex_features, dest=dest_lex,
                                                verbose=False),
    ...
    @staticmethod
    def parse(label, s, weighting=None, cascade=None, directory='', seg_units=[], verbose=False, weight_constraint=None):
        """
...
#                fstpath = os.path.join(directory, fstfile)
                if os.path.exists(fstpath):
                    ...
                    casfile = label + '.cas'
                    if verbose:
                        print('Inserting cascade', label, 'between', src, 'and', dst)
    ...
    def _make_mult_arcs(self, in_string, out_string, src, dst, weight, seg_units):
        ...
        # Last characters in in_strings and out_strings; put weight here.
        self.add_arc(dst1, dst, last_in, last_out, weight=weight)
    ...
    def _make_mult_arcs1(self, in_strings, out_strings, src, label_pre, label_suf, start_index):
        ...
        for index, (s_in, s_out) in enumerate(zip(in_strings, out_strings)):
            ...
            self.add_arc(src1, dst1, s_in, s_out)
            src1 = dst1
        return dst1
    ...
    @staticmethod
    def tree_to_fst(tree, label, cascade=None, weighting=None, lex_features=False, dest=False, weight_constraint=None,
                    verbose=False):
                    ...
                    for n, rest0 in enumerate(rest):
                        ...
                        if lex_features:
                            ...
                            if weight and weight_constraint:
                                constrained_weight = weighting.multiply(weight, weight_constraint)
                                if not constrained_weight:
                                    print('Warning: weight', weight, 'is incompatible with constraint', weight_constraint)
                                else:
                                    weight = constrained_weight
    ...
    def transduce(self, input, init_weight=None, split_string='', join=True, seg_units=[], reject_same=False,
        ...
        for output in self.step_transduce(input, step=False, init_weight=init_weight,
            ...
            if output[1] and (output not in result) and (not reject_same or output[1] != input):
                result.append(output)
                if timeout:
                    # Only count outputs if they succeed
                    if noutputs >= timeout:
                        print(input, 'exceeded', timeout, 'outputs')
                        break
                    noutputs += 1
        res = []
        for r in result:
            if r[2]:
                ...
                res.append([''.join(r[1]) if join else r[1], r[2]])
            else:
                # no weight, just (joined) output string
                res.append(''.join(r[1]) if join else r[1])
        # Group results by output string before returning
        res = self.consolidate(res)
        if timeit:
            print('Transduction took %0.3f seconds' % (time.time()-t1,))
        return res
    ...
    def _transduce_match_input(self, input, in_pos, in_string, weight=None, accum_weight=None,
                               trace=0):
        ...
        if self.is_final(state) and in_pos >= len(input):
            if self.is_weighted():
                ...
                if accum_weight:
                    ...
                    if trace > 1: print('weight:', accum_weight)
                    return accum_weight
            else:
                return True

    def match_unknown(self, target):
        """Does the target string match the unknown character? (MG)"""
    ...
    def step_transduce(self, input, step=True, all_paths=True, init_weight=None,
        ...
        all_paths added to permit returning more than one path (MG).
        """
        output = []
        in_pos = 0
            ...
            while not found and go_on:
                ...
                if trace:
                    ...
                    if in_pos >= len(input):
                        ...
                        print('input pos:', in_pos, 'char:', input[in_pos])
                        if trace > 1: print('  weight:', accum_weight)
...
#                            print('(match)', end=' ')
                        ...
                        if trace > 1 and not input_match:
                            print("  Input match failed")
                        if trace > 1 and (in_pos >= len(input)):
                ...
                if final_match:
                    ...
                    if trace > 2:
                        print(" Updating", arc, state, in_string, out_string, output)
                else:
    ...
    def compose(fsts, label='', relabel=True, trace=0):
        ...
        if len(fsts) > 2:
            ...
            return FST.compose2(comp1, fsts[-1], label=label, relabel=False, trace=trace)
        else:
            return FST.compose2(fsts[0], fsts[1], label=label, relabel=False, trace=trace)
    ...
    def compose2(fst1, fst2, label='', relabel=False, trace=0):
        ...
        if relabel:
            fst = fst.relabeled(label=fst.label)
        if trace:
            print(fst.n_states(), 'states /', fst._n_arcs, 'arcs in', fst.label)
        return fst
    ...
    def really_compose(fst1, fst2, label='', trace=0):
        ...
        if trace:
            print('Composing', fst1.label, 'and', fst2.label)
        ...
        while states:
            if trace > 1:
                print('States', states)
            state_pairs += 1
            if trace and state_pairs % 5000 == 0:
            ...
            for arc1 in fst1.outgoing(state1):
                ...
                for arc2 in fst2.outgoing(state2):
                    ...
                    if comp_in_out_strings:
                        if trace > 1:
                            print(' ARC MATCH', comp_in_out_strings)
                        if fst1.is_weighted():
                            ...
                            if trace > 1 and (weight1 != TOPFSS or weight2 != TOPFSS):
                                print(' Weights', weight1, weight2, 'composed arc wt', composed_weight)
                        if composed_weight:
...
#                                print('Match, in', comp_in, 'out', comp_out)
    ...
    def _composition_ep_preprocess(self, fst2):
        ...
        if not out_ep1 and not in_ep2:
            ...
            fst1 = self.copy(FST.del_suffix(self.label, '.') + '%')
            fst2 = fst2.copy(FST.del_suffix(fst2.label, '.') + '%')
            if in_ep2:
    ...
    def composition_ep_filter(weighting=None, full=True, cascade=None):
        ...
        if cascade:
            ...
            if filt:
                return filt
        return FST.load(os.path.join(EP_FILTER_DIR, label + '.fst'), weighting=weighting, cascade=cascade)
    ...
    def _composition_arc_match(self, fst2, in1, out1, in2, out2, trace=0):
        ...
        if self._label_unknown(out1):
            ...
            if trace > 2 and (stringset1 or stringset2):
                print(' SS: out1', (stringset1 or out1))
                print(' SS: in2', (stringset2 or in2))
...
##                                                                       insertion.out_string(arc)))
    ...
    def del_suffix(string, sufstart, last = True):
        """String without everything after the last (or first) occurrence of sufstart (MG)."""
